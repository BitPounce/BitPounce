...
namespace BitPounce {
	...
	void OpenGLFramebuffer::Invalidate()
	{
...
#ifdef BP_PLATFORM_WEB
		// WebGL only supports single draw buffer
#else
		if (m_ColorAttachments.size() > 1)
		{
			BP_CORE_ASSERT(m_ColorAttachments.size() <= 4, "");
		...
		BP_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
