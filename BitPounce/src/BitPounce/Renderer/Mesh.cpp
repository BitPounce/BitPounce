...
namespace BitPounce
{
	Mesh::Mesh(const std::vector<MeshVertex> &vertices, const std::vector<uint32_t> &indices, const std::vector<TexPair> &textures)
	{
		m_Vertices = vertices;
		m_Indices = indices;
	...
	Ref<Mesh> Mesh::Create(const std::vector<MeshVertex>& vertices, const std::vector<uint32_t>& indices, const std::vector<TexPair>& textures)
	{
